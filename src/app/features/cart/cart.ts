...
export class Cart {
  ...
  private cartItemIds$ = toObservable(this.storageService.cartItemIds);

  private productsResult$ = this.cartItemIds$.pipe(
    switchMap(ids => {
      if (ids.length === 0) {
  ...
  readonly cartProducts = computed(() => this.productsResult().products);
